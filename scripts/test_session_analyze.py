...
def make_assistant_entry(tool_uses=None, usage=None, uuid="u-asst-01"):
    ...
    if usage:
        msg["usage"] = usage
    return json.dumps({"type": "assistant", "uuid": uuid, "sessionId": "test-session-01",
...
def make_user_tool_result(tool_use_id, content, is_error=False, uuid="u-user-01"):
    ...
    return json.dumps({"type": "user", "uuid": uuid, "sessionId": "test-session-01",
...
def single_session_jsonl(session_dir):
    ...
    lines = [
        ...
        make_assistant_entry(
            tool_uses=[{"id": "tu-bash-fail", "name": "Bash",
                        "input": {"command": "cat /nonexistent"}}],
            usage={"input_tokens": 100, "output_tokens": 50,
                   "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
            uuid="asst-1",
        ),
        make_user_tool_result("tu-bash-fail", "cat: /nonexistent: No such file or directory",
                               is_error=True, uuid="user-1"),
        # Turn 2: assistant calls Bash (success)
        make_assistant_entry(
            tool_uses=[{"id": "tu-bash-ok", "name": "Bash",
                        "input": {"command": "ls /"}}],
            usage={"input_tokens": 200, "output_tokens": 30,
                   "cache_read_input_tokens": 500, "cache_creation_input_tokens": 100},
            uuid="asst-2",
        ),
        make_user_tool_result("tu-bash-ok", "bin etc usr", is_error=False, uuid="user-2"),
        # Turn 3: assistant calls Read twice (same file)
        make_assistant_entry(
            tool_uses=[
                {"id": "tu-read-1", "name": "Read", "input": {"file_path": "/home/user/CLAUDE.md"}},
                {"id": "tu-read-2", "name": "Read", "input": {"file_path": "/home/user/README.md"}},
            ],
...
def repeated_sequence_jsonl(session_dir):
    ...
    lines = []
    for i in range(3):
        lines.append(make_assistant_entry(
            tool_uses=[{"id": f"tu-b-{i}", "name": "Bash",
                        "input": {"command": "git status"}}],
            usage={"input_tokens": 100, "output_tokens": 10,
                   "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
            uuid=f"asst-b-{i}",
        ))
        lines.append(make_user_tool_result(f"tu-b-{i}", "On branch main", uuid=f"user-b-{i}"))
        lines.append(make_assistant_entry(
            tool_uses=[{"id": f"tu-r-{i}", "name": "Read",
                        "input": {"file_path": "/etc/hosts"}}],
            usage={"input_tokens": 120, "output_tokens": 5,
                   "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
            uuid=f"asst-r-{i}",
        ))
        lines.append(make_user_tool_result(f"tu-r-{i}", "127.0.0.1 localhost", uuid=f"user-r-{i}"))
...
class TestCwdToProjectPath:
    def test_derives_project_dir_from_cwd(self, session_dir, tmp_path):
        ...
        r = run_script(["--resolve-path", "--cwd", cwd],
                       env={"CLAUDE_PROJECTS_DIR": str(session_dir.parent)})
    ...
    def test_leading_slash_becomes_leading_dash(self, session_dir, tmp_path):
        ...
        r = run_script(["--resolve-path", "--cwd", "/some/path"],
                       env={"CLAUDE_PROJECTS_DIR": str(session_dir.parent)})
        assert r.returncode == 0, r.stderr
        out = json.loads(r.stdout)
...
class TestFailedCommands:
    def test_detects_error_tool_results(self, session_dir, single_session_jsonl):
        r = run_script(["--output-json", "--projects-dir", str(session_dir.parent),
                        "--cwd", "/home/fake/project"],
                       env={})
        assert r.returncode == 0, r.stderr
...
class TestRepeatedSequences:
    def test_detects_repeated_tool_sequences(self, session_dir, repeated_sequence_jsonl):
        r = run_script(["--output-json", "--projects-dir", str(session_dir.parent),
                        "--cwd", "/home/fake/project"],
                       env={})
        assert r.returncode == 0, r.stderr
    ...
    def test_repeated_commands_flagged(self, session_dir, repeated_sequence_jsonl):
        ...
        flat = [item for sub in cmds for item in (sub if isinstance(sub, list) else [sub])]
